    ...
    <div className="p-8 text-white">
      <div className="grid  grid-cols-1 md:grid-cols-2 gap-2 col-span-4">
        ...
        <div className="flex flex-col px-2">
          ...
          <div>
            {" "}
            <p className="text-xl mb-4 text-slate-300" >{projectData.description}</p>
          </div>
          <div className="mt-3">
                  {projectData.techs.map((value:string, index:number) => (
                    <div
                      key={index}
                      className="text-sm mr-2 md:mt-0 mb-6 border-2 border-[#2596be] text-gray-200 px-2 py-1 inline-block rounded-full"
                    >
                      {value}
                    </div>
                  ))}
          ...
          <div className="flex sm:flex-row flex-col justify-start items-start">
            <button className="bg-[#2596be] w-[200px] rounded-md font-medium sm:mr-3 sm:my-6 py-3 text-black">
              <a href={projectData.liveUrl} target="_blank" rel="noreferrer">
                {" "}
                Live Demo
              </a>
            </button>
            <button className="bg-[#2596be] w-[200px] rounded-md font-medium sm:ml-3 sm:my-6 my-4 py-3 p-4 text-black">
